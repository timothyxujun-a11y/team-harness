...
- 技术栈：Java / Spring Boot（Maven 或 Gradle，自动检测）
- AI 工具：Claude Code
- 方案深度：标准层
...
- **可复用**：参数化 + 自动检测 Maven/Gradle 与 Java/Spring Boot 版本，不绑定单一项目
...
│   │   └── test-writer.md     # 子 agent：JUnit5 + Mockito 写单测
│   └── commands/
...
- **自动检测规则**：有 `pom.xml` → Maven；有 `build.gradle` / `build.gradle.kts` → Gradle；Java/Spring Boot 版本从构建文件读取，不硬编码
...
- **测试约定**：JUnit 5 + Mockito + AssertJ；AAA 结构；命名 `shouldXxxWhenYyy`；纯单元测试优先
...
- **test-writer**：为指定类/方法生成 JUnit 5 + Mockito + AssertJ 单测，符合命名与 AAA 约定，优先纯单元测试
...
- **pre-commit**：提交前跑 Checkstyle 静态规则检查（Maven：`checkstyle:check`；Gradle：`checkstyleMain`）；不合规拒绝提交。**只检查不改代码**（非格式化）
  - 若用户后续要求彻底无风格工具，可降级为仅编译检查（`compile`/`classes`）
...
- [ ] `pre-commit` 与 `install-git-hooks.sh` 对 Maven/Gradle 双适配、可执行
